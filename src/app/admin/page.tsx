    ...
    >
      {!adminRow ? (
        <div className="mb-8 rounded-2xl border border-line bg-white p-6">
          <h2 className="text-h3 text-navy">This account isn&rsquo;t an admin yet</h2>
          <p className="mt-3 text-[0.9375rem] leading-relaxed text-slate">
            ...
            is in the <code className="rounded bg-mist px-1.5 py-0.5">admins</code> table.
            Run this once in the Supabase SQL editor:
          </p>
          <pre className="mt-4 overflow-x-auto rounded-xl bg-navy p-4 text-[0.8125rem] text-white">
...
values ('${user.id}', '${user.email ?? ""}');`}
      ...
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <h1 className="text-h2 text-navy">Posts</h1>
      ...
      {posts.length === 0 ? (
        <div className="mt-10 rounded-2xl border border-line bg-white px-8 py-16 text-center">
          <span className="split-bar mx-auto block h-1 w-16 rounded-full" aria-hidden="true" />
          <h2 className="mt-7 text-h3 text-navy">Nothing written yet</h2>
          <p className="mx-auto mt-3 max-w-sm text-[0.9375rem] text-slate">
          ...
          {posts.map((post) => (
            <li key={post.id} className="border-b border-line last:border-0">
              <Link
                ...
                className="flex flex-wrap items-center justify-between gap-4 px-6 py-5 transition-colors hover:bg-mist"
              >
                <div className="min-w-0">
                  <div className="flex items-center gap-3">
                    <span
                      className={`inline-block h-2 w-2 shrink-0 rounded-full ${
                        post.status === "published" ? "bg-green" : "bg-line"
                      }`}
                      aria-hidden="true"
                    />
                    <span className="truncate text-[1.0625rem] font-medium text-navy">
                      {post.title || "Untitled"}
                    </span>
                  </div>
                  <p className="mt-1.5 truncate pl-5 text-[0.8125rem] text-slate">
                    /insights/{post.slug} · updated {formatPostDate(post.updated_at)}
                  </p>
                </div>
                <span className="label shrink-0 text-slate">{post.status}</span>
              </Link>
            </li>
          ))}
        </ul>
